...
// 		funcreturn=funcreturn+(dist2);//*exp(-bez_alpha*dist2); //Sum of reciprocal squared distances, exponentially decaying weight
// 		//Next, find grad for each of five variables
// 		if(grad){
// /* x2 */ 	grad[0]=grad[0]+1.0*2*(bez_curv[i][0]-geo_curv[i][0])*px2;
// /* x3 */ 	grad[1]=grad[1]+1.0*2*(bez_curv[i][0]-geo_curv[i][0])*px3;
// /* y3 */ 	grad[2]=grad[2]+1.0*2*(bez_curv[i][1]-geo_curv[i][1])*py3;
// /* x4 */ 	grad[3]=grad[3]+1.0*2*(bez_curv[i][0]-geo_curv[i][0])*px4;
// /* y4 */ 	grad[4]=grad[4]+1.0*2*(bez_curv[i][1]-geo_curv[i][1])*py4;
// 		}
...
void bezier_inequality_con(unsigned m, double *result, unsigned n, const double* x, double* grad, void* my_func_data){ //Bezier Curve inequalities
	...
	// double x4=xopt[0][7]; //These are fixed by initial conditions and thus aren't variables in optimization
	// double y4=xopt[1][7];
	int num_param_pairs = 7;
	std::vector<std::vector<double>> bez_curv;
	//Optimization variables:
	//[0] -> x2
	//[1] -> x3
	...
	public:
		...
		void lidar_callback(const sensor_msgs::LaserScanConstPtr &data){
			...
			if(drive_state == "normal"){
				...
				int num_fixed = 3;	// Number of bezier curve points that are fully or partially fixed

				for (int num_MPC=0;num_MPC<bez_ctrl_pts-num_fixed;num_MPC++){
				...
				// Set the final point initial guess to the center of the widest gap at the safe distance range

				// Find largest gap in horizon
				...
				// xptplot[1]=safe_distance*cos(theta_ref);
				// yptplot[1]=safe_distance*sin(theta_ref);


				// All lidar scan modifications done here
				// Outputs: fused_ranges_MPC_tot0, lidar_transform_angles_tot0, xptplot
				...
				// opt_params2.push_back(bez_x4); opt_params2.push_back(bez_y4);
				...
				if (optim < 0) {
					safe_distance_adapt=safe_distance_adapt/2;
					ROS_ERROR("Optimization Error, %d, %lf, %lf, %lf\n",optim,x[0],x[1],x[2]);
					...
					double x_dot=4*bez_x1*(-4*pow(t,3)+9*pow(t,2)-6*t+1)+6*x[0]*(4*pow(t,3)-6*pow(t,2)+2*t)+4*x[1]*(-4*pow(t,3)+3*pow(t,2))+4*bez_x4*pow(t,3);
					double x_ddot=4*bez_x1*(-12*pow(t,2)+18*t-6)+6*x[0]*(12*pow(t,2)-12*t+2)+4*x[1]*(-12*pow(t,2)+6*t)+12*bez_x4*pow(t,2);
					//double x_dddot=4*bez_x1*(-24*t+18)+6*x[0]*(24*t-12)+4*x[1]*(-24*t+6)+24*x[3]*t;

					double y_dot=6*bez_y2*(4*pow(t,3)-6*pow(t,2)+2*t)+4*x[2]*(-4*pow(t,3)+3*pow(t,2))+4*bez_y4*pow(t,3);
					double y_ddot=6*bez_y2*(12*pow(t,2)-12*t+2)+4*x[2]*(-12*pow(t,2)+6*t)+12*bez_y4*pow(t,2);
				...
				mpc_marker.type = visualization_msgs::Marker::POINTS;
				mpc_marker.id = 0; 
